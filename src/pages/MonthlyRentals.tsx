...
export default function MonthlyRentals() {
  ...
  const rentalType = "monthly"; // إيجارات شهرية فقط
  ...
  const monthlyRentals = rentals.filter(
    (r) => r.status === "active" && r.rental_type === "monthly"
  );
  const completedMonthlyRentals = rentals.filter(
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-3xl font-bold">الإيجارات الشهرية (الخاصة)</h2>
            <p className="text-muted-foreground">متابعة الإيجارات الشهرية</p>
          </div>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>إيجار شهري جديد</DialogTitle>
              </DialogHeader>
